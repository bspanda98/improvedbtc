...
#define URI_SCHEME "improvedbitcoin"
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a IBTC address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no improvedbitcoin: URI
...
{
    // Convert improvedbitcoin:// to improvedbitcoin:
    //
    //    Cannot handle this later, because improvedbitcoin:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open improvedbitcoin.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "ImprovedbitCoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for ImprovedbitCoin.lnk
...
{
    return GetAutostartDir() / "improvedbitcoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a improvedbitcoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=ImprovedbitCoin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the improvedbitcoin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add improvedbitcoin app to startup item list
